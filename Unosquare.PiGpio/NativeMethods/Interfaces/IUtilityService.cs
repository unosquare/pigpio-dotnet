namespace Unosquare.PiGpio.NativeMethods.Interfaces
{
    ...
    internal interface IUtilityService
    {
        ...
        int GpioTime(TimeType timeType, out int seconds, out int microseconds);
        ...
        int GetBitInBytes(int bitPos, byte[] buf, int numBits);
        ...
        void PutBitInBytes(int bitPos, byte[] buf, int bit);
        ...
        /// <returns>The result code. 0 for success. See the <see cref="ResultCode"/> enumeration.</returns>
        ResultCode Shell(string scriptName, string scriptString);
        ...
        ResultCode GpioSetSignalFunc(uint signalNumber, PiGpioSignalDelegate f);
        ...
        /// <returns>Returns 0 if OK, otherwise PI_BAD_signalNumber.</returns>
        ResultCode GpioSetSignalFuncEx(uint signalNumber, PiGpioSignalExDelegate callback, UIntPtr userData);
        ...
        int RaiseSignal(int signalNumber);
    }
}
