namespace Unosquare.PiGpio.NativeMethods.Interfaces
{
    ...
    public interface IWavesService
    {
        ...
        int GpioWaveAddGeneric(uint numPulses, GpioPulse[] pulses);
        ...
        int GpioWaveAddSerial(UserGpio userGpio, uint baudRate, uint dataBits, uint stopBits, uint offset, uint numBytes, byte[] str);
        ...
        ResultCode GpioWaveDelete(uint waveId);
        ...
        int GpioWaveTxSend(uint waveId, WaveMode waveMode);
        ...
        ResultCode GpioWaveChain(byte[] buffer, uint bufferSize);
